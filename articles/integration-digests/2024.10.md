...
_The Q3 2024 edition of Red Hat's quarterly newsletter on Apache Camel highlighted new releases of Kaoto, the open source visual editor for Apache Camel, and provided a migration guide from Camel K to Camel Extensions for Quarkus. The newsletter also showcased the use of Camel in a live demo at the London Summit Connect, and shared a range of articles and tutorials on topics such as unstructured data extraction with Apache Camel Quarkus and LangChain4j, modernizing Camel's test support code, and efficient data processing with the Chunking DSL._

### Confluent

🔍 [Shift Left: Bad Data in Event Streams, Part 1](https://www.confluent.io/blog/shift-left-bad-data-in-event-streams-part-1/)

_The article discusses the issue of bad data in event streams, which can cause serious problems for data users. It explores the types of bad data, how it can end up in an event stream, and strategies to deal with it, with a focus on prevention through the use of schemas, validation, and testing. The article also highlights the differences between batch processing and event streaming, and the challenges of dealing with bad data in immutable event streams like Apache Kafka._

🔍 [Shift Left: Bad Data in Event Streams, Part 2](https://www.confluent.io/blog/shift-left-bad-data-in-event-streams-part-2/)

_The article discusses strategies for dealing with bad data in event streams, focusing on event design and the "rewind, rebuild, and retry" approach. It explains that state events, which contain the entire statement of fact for a given entity, can be easily fixed by correcting the data at the source, while delta events, which describe a change or an action, are more challenging to correct. The article also highlights the importance of prevention through robust testing, explicit schemas, schema validation, and data quality checks._
...
_The article discusses the importance of observability in modern systems and how to use Debezium for comprehensive database activity logging and analysis in microservice architectures. It explains how Debezium captures row-level changes and streams them in real-time, allowing immediate visibility into database operations. The author also provides a detailed explanation of how to set up Debezium, configure it for specific use cases, and use the generated data to create actionable dashboards. The article concludes by demonstrating how Change Data Capture (CDC) can extract insights from the database to serve as key performance indicators (KPIs) in the reliability and observability of microservices._

### Gravitee

🔍 [Introducing the Gravitee Kafka Gateway](https://www.gravitee.io/blog/introducing-the-gravitee-kafka-gateway)

_Gravitee has introduced the Kafka Gateway, a tool that allows organisations to expose native Kafka directly to the gateway without compromising protocol integrity. The Kafka Gateway offers enhanced security, governance, and simplified management, with features such as authentication mediation, topic mapping, and comprehensive analytics and logging. The tool is designed to make Kafka deployments more secure and controlled, and to bridge the gap between event streams and API management._

🔍 [Introducing Gravitee Platform 4.5 - API Management](https://www.gravitee.io/blog/introducing-gravitee-platform-4.5-apim)

_Gravitee has announced the upcoming release of its platform 4.5, which is designed to help enterprises scale, simplify governance, and accelerate collaboration. The new release includes features such as Shared Policy Groups, which allow organisations to reuse, deploy, and update policies faster, and mTLS Plans, which provide an extra layer of security on top of the gateway. The platform also introduces AI Flows for building intelligent proxies, a Data Cache Policy for storing information like an access token, and an OAS Validation Policy that leverages an OpenAPI specification to run configurable checks on the traffic passing through an API. Additionally, the platform's API Developer Portal has been enhanced with more updates and customisation options._
...
_Gravitee has announced the release of its Access Management 4.5 platform, which introduces mutual TLS (mTLS) support for LDAP upstream Identity Providers (IdPs). This feature enhances security by requiring both the client (Gravitee Access Management) and the server (the user's LDAP) to authenticate each other before any sensitive data is exchanged. This update is particularly beneficial for organisations running Access Management in Gravitee Cloud while maintaining their LDAP servers on-premise, allowing secure authentication and communication between the two environments. The new feature is part of Gravitee's broader strategy to enable secure API management and identity access across distributed infrastructures._
...
_The article introduces updates to Federation and a new feature called API Score. The updates allow for federated API management, enabling the management of APIs and event streams from any vendor. This means that APIs, OpenAPI and AsyncAPI definitions, API products, Kafka schemas, and other metadata from third-party providers can be discovered, ingested, enriched, and published for discovery by developers on the Gravitee Developer Portal. The new API Score feature allows developers to easily obtain an API key to directly consume the underlying API, regardless of whether it's running on IBM API Connect, Solace, or another platform._
...
_Gravitee has launched version 4.5 of its Kubernetes Operator (GKO), which enhances API automation, simplifies GitOps workflows, reduces manual errors, and streamlines microservice deployment. The GKO uses the Kubernetes Operator pattern to automate API management, reducing the risk of configuration drift or misconfigurations, and can also automate the registration of APIs within the Gravitee API Gateway, ensuring each new service is discoverable and secure from the moment it is deployed._
