...
_The blog post discusses the use of windowing in Kafka Streams, a library built on top of Apache Kafka for microservices and streaming applications. The author explains that windowing is used to segment an unbounded dataset, or "stream", to perform stateful operations like joins and aggregations on records with the same key within a specified window boundary. The post provides an overview of the four types of windowing provided by Kafka Streams: Hopping Window, Tumbling Window, Sliding Window, and Session Window. The author also discusses the use of the suppress operator in Kafka Streams to control when the results of a KTable are emitted downstream. The post concludes by suggesting that different window types suit various analytical needs, and that choosing the right window type depends on factors such as the desired analysis granularity, data update frequency, and the specific metrics being tracked._
...
_The author explains that API governance refers to applying policies, rulesets, best practices, and standards to APIs to ensure consistency and effective management, while API reusability refers to reusing APIs to save effort and speed up delivery time. The post provides a step-by-step guide on how to apply API governance in MuleSoft's Anypoint Platform. The author also highlights that API governance ensures that APIs conform to standards and best practices, making them more efficient and effective for reuse. The post concludes by suggesting that API governance and reusability are strongly associated and that governing APIs can ensure more effective reusability._
...
_The blog post announces the general availability of the Telemetry Exporter for Runtime Fabric (RTF) from MuleSoft. The Telemetry Exporter is a service based on the OpenTelemetry standard that allows for the export of real-time telemetry data to a wider observability ecosystem. The author explains that this feature, which was previously only available on CloudHub 2.0, is particularly important for customers using RTF, as it provides access to comprehensive monitoring data across different cloud platforms and environments. The post concludes by noting that the Telemetry Exporter and Distributed Tracing are only available for Mule Runtime 4.6 and above, and provides resources for users to learn more about MuleSoft's monitoring capabilities._

🔍 [Writing RAML Datatype like a PRO](https://medium.com/another-integration-blog/writing-raml-datatype-like-a-pro-0a741e6e36fd)

_The blog post discusses the use of the JSON to RAML Converter, a tool that allows developers to quickly and easily convert JSON examples into RAML datatypes for API documentation. The author explains that creating RAML datatypes manually can be a time-consuming and error-prone process, especially for complex JSON structures. The JSON to RAML Converter automates this process, improving the speed, accuracy, and consistency of RAML datatypes. The tool also offers additional features such as making all keys optional in the RAML output and camel casing all of the keys. The post concludes by suggesting that the JSON to RAML Converter is a valuable tool for any MuleSoft developer who works with JSON data._

### Tyk

🔍 [Performance: Tyk is faster and more performant than Gravitee](https://tyk.io/blog/tyk-vs-gravitee-performance/)

_The blog post discusses the performance advantages of Tyk, an API gateway, over Gravitee. Key points include Tyk's use of Golang for better memory and CPU efficiency, its microservices architecture for precise resource control, and its ability to offload CPU computation of API analytics. The author also notes that Tyk handles traffic spikes better and scales faster from a cold start compared to Gravitee._

## Releases

🚀 [Gravitee 4.4](https://www.gravitee.io/blog/platform-update-gravitee-4.4)

_The blog post announces the 4.4 release of Gravitee, highlighting several new features and updates. Key changes include the introduction of Federated API Management, which allows centralized management of APIs from third-party gateways and event brokers. The release also introduces a new Gravitee Developer Portal with major UI/UX improvements and an Azure Service Bus endpoint that expands "Events as APIs" capabilities. Additionally, the Gravitee Kubernetes Operator has been updated to support V4 APIs and V2 APIs. These changes aim to enhance API management and governance, improve user experience, and expand integration capabilities._

🚀 [Insomnia 9.3 GA](https://konghq.com/blog/product-releases/insomnia-9-3-ga)

_The blog post announces the release of Insomnia 9.3, highlighting several new features and improvements. Key updates include after-response scripting, which allows users to write custom behavior before a request is sent and after a response is received. The release also introduces global environments that can be used across multiple collections and can be stored locally or via Cloud Sync or Git Sync. Additionally, the update includes folder-level settings for authentication, environments, headers, and scripting within collections. Other enhancements include performance improvements and the option to disable end-to-end encryption. The post concludes by promising more exciting capabilities in upcoming versions of Insomnia._

🚀 [Kaoto v2.0](https://camel.apache.org/blog/2024/06/kaoto-release-2.0.0/)

_The blog post announces the release of Kaoto 2.0, a low-code and no-code integration designer based on Apache Camel. The author highlights several new features and improvements, including the ability to create and edit integrations visually or through direct source code access, an extension for Visual Studio Code, commands for creating Camel routes, Kamelets, and Pipes, and a catalog combining various tools for integration. The post also mentions the addition of support for working with expressions and data formats, the ability to enable and disable steps, and new entities for more complex workflows. The author concludes by inviting feedback and contributions from users, and providing a glimpse into the upcoming features planned for Kaoto 2.1._

🚀 [Red Hat build of Apache Camel 4.4](https://developers.redhat.com/blog/2024/06/10/whats-new-red-hat-build-apache-camel-44)

_The blog post announces the release of Red Hat's Apache Camel 4.4, highlighting several new features and improvements. The author discusses the introduction of after-response scripting, which allows users to write custom behavior before a request is sent and after a response is received. The post also introduces the new Kaoto visual designer, which provides a more intuitive no-code/low-code approach to designing integration routes. Other updates include support for new tools, components, and environments, as well as new resources focusing on modernizing enterprise integration applications. The author concludes by providing resources for users to learn more about the new features and updates in Apache Camel 4.4._
